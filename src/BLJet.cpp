...
void BLJet::compute_jet_dynamics() {
    if (verbosity_level > 1) std::cout << "Computing BLJet dynamics" << std::endl;

    // clean up existing gsl interpolation
    if (verbosity_level > 2) std::cout << "cleaning up spline memory" << std::endl;
    if (spline_speed) gsl_spline_free(spline_speed);
...
void BLJet::calc_velocity_profile_magnetized_jet(){
    if (verbosity_level > 1) std::cout << "BLJet::calc_velocity_profile_magnetized_jet" << std::endl;
...
void BLJet::calc_nozzle_energetics_equipartition(){
    if (verbosity_level > 1) std::cout << "BLJet::calc_nozzle_energetics_equipartition" << std::endl;
...
void BLJet::calc_grid_next_zone(size_t i, size_t& cut, double& zcut){
    if (verbosity_level > 1) std::cout << "BLJet::calc_grid_next_zone " << i << std::endl;
    double zinc, z_next;
...
void BLJet::calc_zone_properties(size_t i){
    if (verbosity_level > 1) std::cout << "BLJet::calc_zone_properties " << i << std::endl;
