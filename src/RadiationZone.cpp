...
void RadiationZone::add_target_black_body(double temperature, double energy_density, std::string name) {
    if (verbosity_level > 2) std::cout << "adding black body" << name << " with T="
...
void RadiationZone::compute_radiation() {
    if (verbosity_level > 1) std::cout << "Computing radiation using default energy array" << std::endl;
    int N = 210;
...
void RadiationZone::compute_radiation(std::vector<double> obs_energy_grid) {
    if (verbosity_level > 1) std::cout << "Computing radiation using given energy array" << std::endl;
...
std::vector<double> RadiationZone::get_electron_momentum_grid() {
    const std::vector<double>& vec =
        (fraction_nonthermal_electrons == 0) ? electrons_thermal.get_p() :
        (fraction_nonthermal_electrons < 0.5) ? electrons_mixed.get_p() :
...
std::vector<double> RadiationZone::get_electron_gamma_grid() {
    const std::vector<double>& vec =
        (fraction_nonthermal_electrons == 0) ? electrons_thermal.get_gamma() :
        (fraction_nonthermal_electrons < 0.5) ? electrons_mixed.get_gamma() :
...
std::vector<double> RadiationZone::get_electron_momentum_number_density() {
    ...
    std::vector<double> vec =
        (fraction_nonthermal_electrons == 0) ? electrons_thermal.get_pdens() :
        (fraction_nonthermal_electrons < 0.5) ? electrons_mixed.get_pdens() :
...
std::vector<double> RadiationZone::get_electron_gamma_numbery_density() {
    ...
    std::vector<double> vec =
        (fraction_nonthermal_electrons == 0) ? electrons_thermal.get_gdens() :
        (fraction_nonthermal_electrons < 0.5) ? electrons_mixed.get_gdens() :
...
bool RadiationZone::Compton_calculation_necessary(){
    if (force_compton_calculation) return true;
        ...
        (fraction_nonthermal_electrons == 0) ? electrons_thermal.av_gammasq() :
        (fraction_nonthermal_electrons < 0.5) ? electrons_mixed.av_gammasq() :
